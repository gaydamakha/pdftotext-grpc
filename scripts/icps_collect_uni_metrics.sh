...
RESULTS_FN=$RESULTS_DIR/results-nbwrks-0.txt
...
#Clear the file
>$SERVER_FILE
#Take the first machine in the list
SERVER_AD=${MACHINES[0]}
SERVER_IP=""
...
CODE=1
until [[ CODE -eq 0 ]]; do
    ssh $SERVER_AD $DIRNAME/server_remote_launch.sh -p $SERVER_PORT -b $BIN -c 4096 -a ""
    CODE=$?
    sleep 1
done

echo "$SERVER_AD $SERVER_IP" >>$SERVER_FILE
echo "Success!"
#Fetch server IP
    ...
    for FILENAME in $FIXTURES_DIR/*.pdf; do
        ...
        while [[ $ITER -lt $NB_ITERS ]]; do
            ...
            until [[ $CODE -eq 0 || $TRY -gt $MAX_TRIES ]]; do
                ITER_TIME=$($BIN pdftotext --compress=true --root-certificate $SRC_DIR/certs/localhost.cert \
